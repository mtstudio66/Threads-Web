...
def init_db():
    ...
    try:
        ...
        if not column_exists('users', 'planId'): cursor.execute("ALTER TABLE users ADD COLUMN planId INT DEFAULT NULL")
        ...
        if template_count < 5:
            logger.info("檢測到文案庫資料不足，正在執行寫入預設文案...")
            templates = [
...
def create_template():
    ...
    if not title or not content:
        return jsonify({"success": False, "message": "標題與內容為必填"}), 400
    try:
        db = get_db_connection()
        cursor = db.cursor()
        cursor.execute(
            "INSERT INTO trending_templates (title, category, content) VALUES (%s, %s, %s)",
            (title, category, content)
        )
        db.commit()
        return jsonify({"success": True, "message": "文案新增成功"}), 200
    except Exception as e:
...
def delete_template(template_id):
    try:
        db = get_db_connection()
        cursor = db.cursor()
        cursor.execute("DELETE FROM trending_templates WHERE id = %s", (template_id,))
        db.commit()
        return jsonify({"success": True, "message": "文案已刪除"}), 200
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 500
    finally:
        if cursor: cursor.close()
...
# 在模組層級啟動背景執行緒，確保無論用 Python 還是 Gunicorn 啟動都能執行發文機器人
